  ...
  for (let offset = 0; offset < snapshots.length; offset += INSERT_BATCH_SIZE) {
    ...
    const statements = chunk.map((snapshot) => db.prepare(
      `INSERT OR IGNORE INTO forecast_snapshots (
        ...
        issued_at, model_run_at, valid_at, lead_hours,
        grid_latitude, grid_longitude,
        wave_height, wave_direction, wave_period, wave_peak_period,
        total_swell_height, total_swell_direction, total_swell_period, total_swell_peak_period,
        swell_height, swell_direction, swell_period, swell_peak_period,
        secondary_swell_height, secondary_swell_direction, secondary_swell_period,
        tertiary_swell_height, tertiary_swell_direction, tertiary_swell_period,

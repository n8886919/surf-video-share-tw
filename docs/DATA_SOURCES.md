...
Open-Meteo ingestion always calls the normal live Marine Forecast endpoint. MFWAM requests 126 future hours at UTC 00:20/12:20 (five selectable Taipei dates plus rollover buffer); at UTC 06:20/18:20 it requests `48 - currentTaipeiHour` future hours, through tomorrow 23:00; the three collect-only models request one future hour. All four also request `past_hours=6`. Every requested MFWAM hour must contain wave height/direction/period before the spot counts as complete.

The official [model update documentation](https://open-meteo.com/en/docs/model-updates) and its live page identify `https://marine-api.open-meteo.com/data/meteofrance_wave/static/meta.json`. Read once per collection; validate timestamps and wait out the documented ten-minute replication interval before using the availability triple as a deduplication hint. The point contents, grid, valid time, provider/model, spot and snapshot kind remain part of the stable key. The first retained retrieval time remains unchanged; new versions or changed point values create new rows. Metadata errors never stop recent-past collection. `model_run_at` remains null because metadata does not prove the run of every hourly response value. Transient HTTP 502/503/504 without Retry-After receive at most one immediate retry; 429 and Retry-After are not retried in a loop. No inferred publication minute or self-learning scheduler is introduced. CWA Home Assistant scheduling and downloads are unchanged; Worker ingestion now checks incoming stable IDs through the primary index before submitting missing inserts, while completion retains its existing covering-index check.

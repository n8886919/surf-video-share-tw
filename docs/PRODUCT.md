...
- 找浪先固定顯示所選時間的目前預報，旁邊以可橫向滑動的卡片呈現同來源候選影片縮圖與拍攝當時預報。使用者先比較再選一支展開，只有按下播放才建立短效播放網址；選擇候選片本身不建立播放器或預載影片。
- 上傳 5–60 秒、原始檔最多 200 MB（200,000,000 bytes）、拍攝時間不得晚於現在且不得早於現在 168 小時以上。瀏覽器不先轉檔；原始檔直接上傳後由影音供應商處理。
...
- 全站提供不打斷主要流程的「問題回報」入口。訪客只填 5–300 字問題描述；系統自動附上所在分頁，不要求登入、聯絡方式或其他個資。
...
- 「我的」只在本人完整、已就緒且仍公開的影片顯示分享功能。第一次操作準備 Stream 轉出的 MP4，完成後由使用者第二次點擊開啟系統分享選單；不支援檔案分享、檔案過大或跨來源讀取失敗時，保留「下載 MP4（非原始檔）」退路。分享目標由作業系統決定，不承諾 LINE、Instagram 或特定 App 一定出現。

## Navigation

手機底部固定三個入口：左側「彼日浪影」、中央較大的「上傳」、右側「我的」。個人設定放在「我的」內，不增加第四個分頁；問題回報是底部導覽上方的次要入口，不占用分頁。

...
- The six-hour maintenance Cron remains `20 */6 * * *` UTC and is responsible for Open-Meteo ECMWF WAM, expired-video cleanup, and playback-event cleanup. The separate hourly operations-analysis Cron is `5 * * * *`. Neither downloads or parses the CWA archive; `CWA_QUERY_STRING_REDACTION_VERIFIED` remains `false` as a retired defense-in-depth guard.
...
- The App first authenticates `GET /api/v1/internal/forecast-ingestion/spots`, so active spot IDs and coordinates remain server-authoritative. It then streams and bounds the F-A0020-001 ZIP, fetches F-A0021-001 locations `O00400,10002030,O01200,O01300,B02400,O00700` with the CWA key in the Authorization header, maps each active spot through the reviewed fixed table, and sends signed v2 batches of at most five to `POST /api/v1/internal/forecast-ingestion/cwa`.
- Requests use HTTPS plus HMAC-SHA256 over signature version, timestamp, nonce, method, exact pathname, and raw-body SHA-256. The Worker allows at most five minutes of clock skew, caps the body at 128 KiB, validates the fixed CWA contract, active spots, and exact v2 spot/LocationId allowlist, recomputes stable IDs, chooses the receive/retrieve timestamp, and writes with `INSERT OR IGNORE`. Replaying the same upstream run must report duplicates rather than create new rows. During the App `0.1.x` to `0.2.0` rollout, deploy the backward-compatible Worker first; it accepts pending v1 batches without granting v1 access to the new six-spot tide mappings.
...
- Each Open-Meteo request still asks for 168 hourly ECMWF WAM forecast hours for immutable provenance. Public matching accepts Taipei day offsets 0–4 at 05:00–19:00: offsets 0–2 require both ECMWF and CWA, while offsets 3–4 are explicitly ECMWF-only. CWA remains independently bounded to verified 0–72-hour leads. New v2 runs retain approved tide fields for all eight spots; old rows remain immutable and are never repaired from a later tide forecast.

...
- The E2E exposed Stream's transient `duration: 0` during initial processing. A local, not-yet-deployed fix now treats non-positive duration as unavailable, waits for a positive real-provider duration before publication, and fails closed when a later verified duration is outside 5–60 seconds. Owner-list reconciliation uses the same rule.
...
- The first completion attempt failed because Stream transiently returned `duration: 0`. The owner-authorized recovery conditionally updated exactly one D1 row after Stream reported `ready` and `20.1` seconds. The code fix is local only and must be reviewed/deployed before another real upload.
- A duplicate private upload remains intentionally untouched pending explicit deletion approval: D1 video `08d8273d-ff01-4d01-bd45-e4605e0ace7b`, Stream UID `04c511182f64c51ac6c9955f8a3f1fe3`, status `awaiting_upload`, `public_at = NULL`. Stream deletion and D1 cleanup have not been tested.
- Stream deletion and webhook behavior still need staging or a separately approved safe verification.
...
Objective: review and publish the transient-duration hardening, then clean the exact duplicate private upload with separate explicit approval.

Scope:

- Review `src/worker/video-status.ts`, its use by completion/reconciliation, and the five regression tests. Deploy only after explicit approval.
- After the fixed version is active, verify the deployed artifact and retain the successful public video unchanged.
- With separate explicit deletion approval, delete only Stream UID `04c511182f64c51ac6c9955f8a3f1fe3`, confirm absence, and then delete only D1 video `08d8273d-ff01-4d01-bd45-e4605e0ace7b`; do not touch the public video.
- Then verify upload/playback burst guards non-destructively. Exercise reporting/delisting, seven-day cleanup, and webhook behavior only in staging or with another reviewed safe procedure; do not shorten production expiry on user data.
...
- The duration fix is reviewed, deployed, and post-deploy checks pass.
- The exact duplicate is removed from both Stream and D1 without changing the successful public video.
- The observed provider timing/origin behavior remains documented without exposing credentials.

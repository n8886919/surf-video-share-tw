...
Product `0.25` temporary LINE diagnostics is implemented under the owner's explicit implement/deploy request and ready for the reviewed local OAuth deployment. Full `pnpm verify` passed lint, typecheck, 38 files / 248 tests, migration drift, production build, 2 rendered-site tests and 13 Chromium/accessibility tests. Default auto login, manual retry, one-use OAuth/nonce/PKCE, seven-day sessions and cookie security remain unchanged. Migration 0016 adds only an isolated diagnostic table/indexes; collection stops on September 14 at 08:00 Taipei and bounded seven-day-target cleanup uses the existing hourly Cron. Preflight confirms required secrets/redaction are present; only migration 0016 and its deadline var are pending. Physical-mobile acceptance is not established; publication/production read-back follows this checkpoint.
...
Product `0.24` implementation commit `f8f425e` is pushed to `main` and deployed through the reviewed local OAuth flow on 2026-09-06. The active Cloudflare version is `a802d796-db18-4598-8eda-6852f5a3ac69` at 100% (`2026-09-06T01:06:07Z`). Migration 0015 and query-string redaction are verified. The initial Workers Build failed on September 5; its exact failing step remains unverified, but it no longer blocks this release.
...
- September 7 follow-up: the user reports Android/Chrome failure with three 17:09 screenshots, while desktop Chrome QR login succeeds. Production has owner sessions created at 17:09:11, 17:10:22 and 17:10:58 Taipei, each valid until September 14; device attribution awaits the user's operation timeline. Health/readiness are currently 200/ok and no ops events were found in the bounded 16:55–17:15 window. This does not establish that the failed mobile callback succeeded.
...
Finish the Product `0.25` release gate and reviewed OAuth deployment, verify a credential-free diagnostic callback and `/me` event in production D1, then obtain one original-entry Android retry with its diagnostic ID and Taipei time. The user confirmed 17:09 was mobile and does not know the LINE password, so do not require manual/password login. Use actual trace evidence to select a fix, with physical Android and iPhone acceptance required; whole-day D1 usage remains a separate observation.

...
Release follow-up: Product `0.25` commit `6c94ea5` was published locally as Cloudflare version `0b198368-8940-46ce-8206-351c957bf97f`; migration 0016 and redaction succeeded. Final integration review found that the pre-existing Worker entry did not forward its execution context to Hono, causing the diagnostic fallback to wait for storage. Product `0.26` forwards that context and adds a built-Worker non-blocking-response regression. Its fresh full `pnpm verify` passed lint, typecheck, 38 files / 248 tests, migration drift, build, 3 built-Worker/rendered-site tests and 13 Chromium/accessibility tests; replacement deployment is next. This is a diagnostic integration fix, not evidence of the user's mobile root cause. No commit from this local release has been pushed.

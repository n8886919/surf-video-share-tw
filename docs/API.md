...
| Internal HMAC | POST | `/internal/forecast-ingestion/cwa/complete` | Verify that one CWA run covers every active spot, then send or deduplicate its owner LINE notification |

The three forecast-ingestion routes are not browser APIs. Every request requires the dedicated `FORECAST_INGESTION_SECRET` and HMAC-SHA256 headers covering signature version, Unix timestamp, nonce, uppercase method, exact pathname, and the SHA-256 of the raw body. The accepted clock skew is at most five minutes. Missing configuration fails closed; malformed, expired, wrong-path, wrong-signature, or over-128-KiB requests are rejected before D1. The row-ingestion POST contract is `cwa-forecast-ingestion-v4`: payload version 4 accepts only provider `cwa`, model `cwa-wave-f-a0020-001`, batches of at most five, three-hourly leads 0–72, active server-known spots, bounded nullable metrics, and fixed F-A0020-001 plus the nearest allowlisted per-spot F-A0021-001 provenance. A mismatched spot/LocationId is rejected before D1. Payload versions 1, 2, and 3 remain temporarily accepted for a safe Worker-first rollout and retain their former tide mappings. The Worker normalizes timestamps, recomputes the stable forecast ID, assigns receive/retrieve time, and returns only `attempted`, `inserted`, and `duplicates`. The completion POST accepts only the pinned CWA provider/model plus one bounded issued/model-run relationship; it independently verifies stored coverage before attempting LINE.

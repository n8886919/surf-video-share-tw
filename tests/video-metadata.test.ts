...
describe("bounded client video metadata", () => {
  ...
  it("accepts 19:59 and rejects capture-time hints outside 05:00–19:59 in Taipei", () => {
    const now = new Date("2026-08-30T12:00:00.000Z");
    expect(selectCaptureTimeHint(
      { recordedAt: new Date("2026-08-30T11:59:59.999Z"), containerCreatedAt: null },
      0,
      now,
    )).toMatchObject({ source: "recording-metadata" });
    expect(selectCaptureTimeHint(
      { recordedAt: new Date("2026-08-30T12:00:00.000Z"), containerCreatedAt: null },
      0,
      now,
    )).toBeNull();
  });
